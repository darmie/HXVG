...
abstract Edges(Array<Edge>) {
    ...
    @:from
    public static inline function fromVector(v:Vector<Edge>):Edges return new Edges(v.toArray());
...
class TrueType {
	public static function rasterize(result:Bitmap, flatnessInPixels:Float, vertices:Array<Vertex>, scaleX:Float, scaleY:Float, shiftX:Float, shiftY:Float,
    ...
    static function tt_rasterize(result:Bitmap, pts:Array<Point>, wcount:Array<Int>, windings:Int, scaleX:Float, scaleY:Float, shiftX:Float, shiftY:Float, offX:Int, offY:Int, invert:Bool){
        var yScaleInv:Float;
        if (invert) {
            yScaleInv = -scaleY;
        } else {
	...
	public static function flattenCurves(vertices:Array<Vertex>, objspaceFlatness:Float):{windings:Array<Point>, lengths:Array<Int>, count:Int} {
    ...
    public static function closeShape(vertices:Array<Vertex>, numVertices:Int, wasOff:Bool, startOff:Bool, sx:Int, sy:Int, scx:Int, scy:Int, cx:Int, cy:Int){
        if (startOff) {
            if (wasOff) {
                vertices[numVertices] = {type: vcurve, x: (cx + scx) >> 1, y: (cy + scy) >> 1, cx: cx, cy: cy};
                numVertices++;
            }
            vertices[numVertices] = {type: vcurve, x: sx, y: sy, cx: scx, cy: scy};
            numVertices++;
        } else {
            if (wasOff) {
                vertices[numVertices] = {type: vcurve, x: sx, y: sy, cx: cx, cy: cy};
                numVertices++;
            } else {
                vertices[numVertices] = {type: vline, x: sx, y: sy, cx: 0, cy: 0};
                numVertices++;
            }
        }
        return numVertices;
